...
const images = [
  "/images/products/900a/main.jpg",
  "/images/products/900a/detail1.jpg",
  "/images/products/900a/detail2.jpg",
  "/images/products/900a/detail3.jpg",
  "/images/products/900a/detail4.jpg",
  "/images/products/900a/detail5.jpg",
  "/images/products/900a/detail6.jpg",
  "/images/products/900a/detail7.jpg",
];

const features = [
  { title: "Stainless Steel Panel", desc: "Durable and smooth for professional ironing results" },
  { title: "Compact Design", desc: "Portable and lightweight for easy storage" },
  { title: "Fast Heating", desc: "First steam ready in just 25 seconds" },
  { title: "3 Steam Gears", desc: "Adjustable output for different fabric types" },
];
...
const specs = [
  ...
  ["Water Tank", "200ml"],
  ["Steam", "3 Gears"],
  ["Continuous Steam", "20±5g/min"],
  ["Steam Outlet", "6 Holes"],
  ["Product Size", "98.5 x 74 x 248 mm"],
  ["First Steam", "25 Seconds"],
...
export default function P900APage() {
  ...
  const relatedItems = [{ model: "ST-815", img: "/images/products/ST-815.png", desc: "1600W handheld steamer with 340ml tank" },{ model: "6617", img: "/images/products/6617.jpg", desc: "1800W steamer with ceramic panel" },{ model: "6618", img: "/images/products/6618.jpg", desc: "Foldable steamer with 3 steam modes" }];
    ...
    <div className="bg-[#050507] text-white min-h-screen">
      ...
      <section className="max-w-6xl mx-auto px-6 py-8 grid gap-10 lg:grid-cols-2 items-start">
        <div>
          ...
          <h1 className="text-5xl md:text-6xl font-black tracking-tight">900A</h1>
          <h2 className="text-xl text-white/70 mt-2">Compact Garment Steamer</h2>
          <p className="text-white/50 mt-2 text-sm">Professional Steam For Modern Care</p>
          <div className="mt-6 flex gap-4">
            {[["200ml", "Tank"], ["20g/min", "Steam"], ["25s", "Heat-Up"]].map(([val, label]) => (
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/contact?product=900A" className="px-5 py-3 bg-[#ff2f7d] text-white font-semibold rounded-full text-sm hover:bg-[#e6006f] transition">
              Request Quote
            </Link>
            <a href="/specs/900a-spec.pdf" download className="px-5 py-3 border border-white/20 text-white font-semibold rounded-full text-sm hover:border-white/50 transition inline-flex items-center gap-2">
        ...
        <div className="bg-[#0d0d12] rounded-2xl overflow-hidden border border-white/8">
          <div className="aspect-square relative" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
            <Image src={images[imgIdx]} alt="900A compact garment steamer with stainless steel panel, XINGYUE factory" fill className="object-contain p-4" unoptimized />
      ...
      <section className="max-w-6xl mx-auto px-6 py-16 text-center">
        ...
        <Link href="/contact?product=900A" className="mt-8 inline-block px-8 py-3 bg-[#ff2f7d] text-white font-semibold rounded-full text-sm hover:bg-[#e6006f] transition">Request OEM Quote</Link>
      </section>
...
<section className="border-t border-white/8 py-20 text-center px-6">
        <h2 className="text-3xl font-bold">Interested in 900A?</h2>
        <p className="mt-3 text-white/50 max-w-md mx-auto">Get factory pricing and customization options.</p>
        <Link href="/contact?product=900A" className="mt-8 inline-block px-8 py-3 bg-[#ff2f7d] text-white font-semibold rounded-full text-sm hover:bg-[#e6006f] transition">Send Inquiry</Link>
      </section>
    </div>
  );
}
