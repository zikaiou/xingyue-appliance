...
const images = [
  "/images/products/sts1004/main.jpg",
  "/images/products/sts1004/detail1.jpg",
  "/images/products/sts1004/detail2.jpg",
  "/images/products/sts1004/detail3.jpg",
];

const features = [
  { title: "2800W Powerful Steam", desc: "Quick heat-up and strong steam flow for professional garment results." },
  { title: "330ml Water Tank", desc: "Transparent large water tank for long ironing sessions." },
  { title: "Self-Clean System", desc: "Maintains peak performance by automatically removing mineral build-up." },
  { title: "Premium Purple Finish", desc: "Elegant design with high-quality ergonomic handle." },
];
...
const specs = [
  ...
  ["Water Tank", "330ml"],
  ["Special Function", "Self-Clean, Anti-Calc"],
  ["Soleplate", "Ceramic"],
];

const scenes = ["Home Use", "Hotel Room", "Online Brand", "Appliance Distributor"];
const oemItems = ["Logo Customization", "Color Options", "Packaging Design", "Mass Production Support"];

export default function STS1004Page() {
    ...
    <div className="bg-[#050507] text-white min-h-screen font-sans">
      ...
      <section className="max-w-6xl mx-auto px-6 py-8 grid gap-10 lg:grid-cols-2 items-start product-hero-grid">
        <div className="reveal is-visible">
          ...
          <h1 className="text-5xl md:text-6xl font-black tracking-tight">ST-S1004</h1>
          <h2 className="text-xl text-white/70 mt-2">Self-Clean Steam Iron</h2>
          <p className="text-white/50 mt-2 text-sm">Professional Performance, Easy Maintenance</p>
          ...
          <div className="mt-8 flex flex-col sm:flex-row sm:flex-wrap gap-3">
            <Link href="/contact?product=ST-S1004" className="w-full sm:w-auto justify-center px-5 py-3 bg-[#ff2f7d] text-white font-semibold rounded-full text-sm hover:bg-[#e6006f] transition inline-flex items-center gap-2">
              Request Quote
            </Link>
            <a href="/specs/st-s1004-spec.pdf" download className="w-full sm:w-auto justify-center px-5 py-3 border border-white/20 text-white font-semibold rounded-full text-sm hover:border-white/50 transition inline-flex items-center gap-2">
        ...
        <div className="bg-[#0d0d12] rounded-2xl overflow-hidden border border-white/8 reveal is-visible product-gallery">
          <div className="aspect-square relative" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
            <Image src={images[imgIdx]} alt="ST-S1004 self-clean steam iron 2800W with anti-calc, XINGYUE manufacturer" fill sizes="(max-width:1023px) 100vw, 50vw" className="object-contain p-4" />
            {images.length > 1 && (
              <>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16 text-center">
        ...
        <Link href="/contact?product=ST-S1004" className="mt-8 inline-block px-8 py-3 bg-[#ff2f7d] text-white font-semibold rounded-full text-sm hover:bg-[#e6006f] transition">Request OEM Quote</Link>
      </section>

            <RelatedProducts title="Related Products"  />

<section className="border-t border-white/8 py-20 text-center px-6">
        <h2 className="text-3xl font-bold">Interested in ST-S1004?</h2>
        <p className="mt-3 text-white/50 max-w-md mx-auto">Get factory pricing and customization options.</p>
        <Link href="/contact?product=ST-S1004" className="mt-8 inline-block px-8 py-3 bg-[#ff2f7d] text-white font-semibold rounded-full text-sm hover:bg-[#e6006f] transition">Send Inquiry</Link>
      </section>
    </div>
  );
}
