...
const images = [
  "/images/products/6618/main.jpg",
  "/images/products/6618/detail1.jpg",
  "/images/products/6618/detail2.jpg",
  "/images/products/6618/detail3.jpg",
  "/images/products/6618/detail4.jpg",
  "/images/products/6618/detail5.jpg",
];

const features = [
  { title: "Stainless Steel Panel", desc: "Durable smooth surface for professional care" },
  { title: "3 Steam Levels", desc: "Adjustable steam for different fabric types" },
  { title: "Compact Design", desc: "Foldable and portable for easy storage and travel" },
  { title: "Fast Steam", desc: "First steam ready in just 25 seconds" },
];
...
const specs = [
  ...
  ["Product Size", "17 x 12 x 23 cm"],
  ["Water Tank", "150ml"],
  ["Frequency", "50/60Hz"],
  ["Plug", "EU Plug"],
  ["Cable", "1.5m"],
  ["Steam", "3 Gears"],
  ["Continuous Steam", "15±5g/min"],
  ["First Steam Time", "25 seconds"],
...
export default function P6618Page() {
    ...
    <div className="bg-[#050507] text-white min-h-screen">
      ...
      <section className="max-w-6xl mx-auto px-6 py-8 grid gap-10 lg:grid-cols-2 items-start product-hero-grid">
        <div>
          ...
          <h1 className="text-5xl md:text-6xl font-black tracking-tight">6618</h1>
          <h2 className="text-xl text-white/70 mt-2">Compact Garment Steamer</h2>
          <p className="text-white/50 mt-2 text-sm">Compact Steam Care For Everyday Use</p>
          <div className="mt-6 flex gap-4">
            {[["150ml", "Tank"], ["15g/min", "Steam"], ["25s", "Heat-Up"]].map(([val, label]) => (
          ...
          <div className="mt-8 flex flex-col sm:flex-row sm:flex-wrap gap-3">
            <Link href="/contact?product=6618" className="w-full sm:w-auto justify-center px-5 py-3 bg-[#ff2f7d] text-white font-semibold rounded-full text-sm hover:bg-[#e6006f] transition inline-flex items-center gap-2">
              Request Quote
            </Link>
            <a href="/specs/6618-spec.pdf" download className="w-full sm:w-auto justify-center px-5 py-3 border border-white/20 text-white font-semibold rounded-full text-sm hover:border-white/50 transition inline-flex items-center gap-2">
        ...
        <div className="bg-[#0d0d12] rounded-2xl overflow-hidden border border-white/8 product-gallery">
          <div className="aspect-square relative" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
            <Image src={images[imgIdx]} alt="6618 foldable garment steamer with 3 steam modes, professional care" fill sizes="(max-width:1023px) 100vw, 50vw" className="object-contain p-4" />
      ...
      <section className="max-w-4xl mx-auto px-6 py-16">
        <h2 className="text-2xl font-bold text-center">Experience 6618</h2>
        <p className="text-center text-white/40 text-sm mt-2 mb-8">Compact Steam Care For Everyday Use</p>
        <div className="rounded-2xl overflow-hidden border border-white/8 bg-black max-w-2xl mx-auto">
          <video controls className="w-full aspect-video" poster="/images/products/6618/main.jpg" preload="metadata">
            <source src="/videos/6618.mp4" type="video/mp4" />
          </video>
        </div>
      </section>
...
<section className="border-t border-white/8 py-20 text-center px-6">
        ...
        <Link href="/contact?product=6618" className="mt-8 inline-block px-8 py-3 bg-[#ff2f7d] text-white font-semibold rounded-full text-sm hover:bg-[#e6006f] transition">Send Inquiry</Link>
      </section>
    </div>
  );
}
