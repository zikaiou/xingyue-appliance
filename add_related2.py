...
for d, cat in dir_cat.items():
    ...
    if "RelatedProducts" in content and "relatedItems" in content:
    ...
    # find Final CTA comment (any numbering)
    m = re.search(r'\s*\{/\* \d+\. Final CTA[^*]*\*/\}', content)
    if not m:
        print("CTA NOT FOUND:", d)
        continue
    marker = m.group(0)
    insert = f'      <RelatedProducts title="Related Products" products={{relatedItems}} />\n'
    content = content.replace(marker, insert + marker, 1)

    # inject const before return
    content = content.replace(
