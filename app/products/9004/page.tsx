      ...
      <section className="max-w-6xl mx-auto px-6 py-8 grid gap-10 lg:grid-cols-2 items-start product-hero-grid">
        <div className="reveal is-visible">
          ...
          <div className="mt-8 flex flex-col sm:flex-row sm:flex-wrap gap-3">
            <Link href="/contact?product=9004" className="w-full sm:w-auto justify-center px-5 py-3 bg-[#ff2f7d] text-white font-semibold rounded-full text-sm hover:bg-[#e6006f] transition inline-flex items-center gap-2">
              Request Quote
            </Link>
            <a href="/specs/9004-spec.pdf" download className="w-full sm:w-auto justify-center px-5 py-3 border border-white/20 text-white font-semibold rounded-full text-sm hover:border-white/50 transition inline-flex items-center gap-2">
        ...
        <div className="bg-[#0d0d12] rounded-2xl overflow-hidden border border-white/8 reveal is-visible product-gallery">
          <div className="aspect-square relative" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
            <Image src={images[imgIdx]} alt="9004 vacuum garment steamer with rotatable ironing head, 1300W" fill sizes="(max-width:1023px) 100vw, 50vw" className="object-contain p-4" />
          </div>
        </div>
      </section>
